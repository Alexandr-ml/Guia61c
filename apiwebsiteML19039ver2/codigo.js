...
function insertar() {
    ...
    if(!(input_title.value === "" && input_description.value === "" && input_price.value === "" && input_category.value === "")) {
        ...
        fetch("https://retoolapi.dev/hhJcOh/productos",
...
function borrar(boton) {
	...
	fetch("https://retoolapi.dev/hhJcOh/productos/" + id,{method:"DELETE"})
...
function obtenerProductos() {
	fetch("https://retoolapi.dev/hhJcOh/productos").then(res=>res.json()).then(
		data=>{
			productos=data;
			productos.forEach(
			function(producto) {
